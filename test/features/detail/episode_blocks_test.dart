...
void main() {
  ...
  group('when there is nothing to jump over', () {
    ...
    test('nonsense input produces nothing rather than throwing', () {
      expect(
        episodeBlocks(total: 0, size: 50, descending: false, firstNumber: 1),
        isEmpty,
      );
      expect(
        episodeBlocks(total: 100, size: 0, descending: false, firstNumber: 1),
        isEmpty,
      );
    });
  });
        ...
        for (final b in blocks) {
          ...
          for (var n = lo; n <= hi; n++) {
            expect(
              covered.add(n),
              isTrue,
              reason: '$n is in two blocks (desc=$desc)',
            );
          }
        }
        expect(
          covered.length,
          237,
          reason: 'every episode is reachable (desc=$desc)',
        );
      }
    });
