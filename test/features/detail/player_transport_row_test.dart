...
void main() {
  for (final width in [320.0, 412.0, 800.0]) {
    for (final previous in [true, false]) {
      testWidgets(
        'next stays right of seek-forward at width $width, previous=$previous',
        (tester) async {
          tester.view.physicalSize = Size(width, 240);
          tester.view.devicePixelRatio = 1;
          addTearDown(tester.view.resetPhysicalSize);
          addTearDown(tester.view.resetDevicePixelRatio);
          Widget button(String key, {bool large = false}) => SizedBox(
            key: ValueKey(key),
            width: large ? 72 : 52,
            height: large ? 72 : 52,
          );
