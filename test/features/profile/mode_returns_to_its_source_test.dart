...
void main() {
  ...
  group('HiveService per-mode memory', () {
    ...
    test('each mode keeps its own source', () async {
      await hive.saveLastProviderForMode('video', 'cs:hdrezka');
      await hive.saveLastProviderForMode('manga', 'mn:mangadex');
      await hive.saveLastProviderForMode('video', 'an:gogo');
      expect(hive.getLastProviderForMode('video'), 'an:gogo');
      expect(hive.getLastProviderForMode('manga'), 'mn:mangadex');
      expect(hive.getLastProviderForMode('novel'), '');
    });
  });
}
